...
class MPCQueue
{
 public:
  ...
  void updateQueue();
  void checkPathReferences(Eigen::Vector3d position_ref, double yaw_ref);
  void publishPathReferences();

  void setRemainingPath(const mav_msgs::EigenTrajectoryPoint& point);
  void setRemainingPath(const mav_msgs::EigenTrajectoryPointDeque& trajectory);
 ...
 private:
  ...
  ros::Publisher remaining_path_publisher;
  ros::Publisher assumed_pose_publisher;
